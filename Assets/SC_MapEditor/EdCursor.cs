...
/// Минимальный шаг сетки 0.25, тогда если GRID_STEP==2, то сетка 0.5 метра. 
/// для метровой сетки нужно 4
/// GRID можно вытащить в отдельный объект. GRID, это просто модификатор для Vec3
/// Сам курсор автономен. В результате чего он будет двигаться (мышь, клавиши, свайпы) для
/// внешней системы значения не имеет
/// </summary>
...
public interface IEdCursor
{
    ...
    void SetGridSize(float gridSize); // TODO: DI
}
...
public class EdCursor : MonoBehaviour, IEdCursor
{
    ...
    public void OnDragDelegate(BaseEventData data)
    {
        ...
        var pd = data as PointerEventData; // приведение типов. ппц
        ...
        if (mPrevCursorPos != mCursorPos)// event notify
        {
