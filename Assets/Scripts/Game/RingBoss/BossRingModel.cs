...
namespace RBGame
{
    ...
    public class BossRingModel : GModel, IBoss
    {
        ...
        private void Start ()
        {
            transform.position = (cx.TripManager.FinishDist + 30) * Vector3.forward;

            CurrHP = PrevHP = MaxHP = cx.BallsManager.TotalCount * cx.BossInfo.hp + 1; // +1 не бывает босcа с 0 HP
        ...
        void CreateRing ()
        {
            ...
            foreach (var e in cx.CurrStage.bossInfo.brona)
            {
        ...
        void OnRedCubeHit (Collider obj)
        {
            ...
            if (cx.BattleManager.PlayerHP <= 0)
                return;
