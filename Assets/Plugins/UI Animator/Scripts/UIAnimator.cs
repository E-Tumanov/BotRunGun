...
namespace UIAnimatorCore
{
	...
	public class UIAnimator : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler, IPointerClickHandler
	{
		...
		[SerializeField]
		private ActionMatrixData m_onPointerClickAMD = new ActionMatrixData(false);
...
#pragma warning restore 0414
		...
		private void Awake()
		{
			if(TryGetComponent<UnityEngine.UI.Button>(out UnityEngine.UI.Button butt))
			{
				m_Button = butt;
			}
		}
		...
		{
			if (isInteracteble && m_onPointerClickAMD.IsEnabled)
			{
				PlayAnimation (m_onPointerClickAMD.AnimType, m_onPointerClickAMD.Delay);
			}
		}
		...
		private void LateUpdate ()
		{
			...
			if(m_Button && m_Button.interactable != isInteracteble)
				isInteracteble = m_Button.interactable;
		}
		...
		public void SetPlayOnPointerClick(bool a_playOnPointerClick, AnimSetupType a_animToPlay)
		{
			SetPlayOnPointerClick (a_playOnPointerClick, a_animToPlay, 0);
		}

		public void SetPlayOnPointerClick(bool a_playOnPointerClick, AnimSetupType a_animToPlay, float a_delay)
		{
			m_onPointerClickAMD.IsEnabled = a_playOnPointerClick;
			m_onPointerClickAMD.AnimType = a_animToPlay;
			m_onPointerClickAMD.Delay = a_delay;
		}
